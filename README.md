...
- Login com perfis separados de administrador e vendedor
- Criação de vendedor pelo administrador
...
1. Crie um usuário no **Authentication > Usuários**.
2. Vá ao Firestore.
3. Crie ou ajuste o documento em `users/{uid}` desse usuário.
4. Altere:
   - `role: 'admin'`
   - `status: 'active'`
...
- O cadastro público de vendedor foi removido: novos acessos são criados pelo administrador na área **Vendedores**.
