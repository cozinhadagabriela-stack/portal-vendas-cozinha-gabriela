...
export function buildNavigation(profile) {
  ...
  return [
    ...
    { id: 'vendor-purchases', label: 'Fornecedor' },
...
export function renderPurchasesView(stockSummary = [], purchases = [], sales = [], editingStockItem = null) {
  const totalAvailable = stockSummary.reduce((acc, item) => acc + Number(item.availableQuantity || 0), 0);
  const totalPurchased = purchases.reduce((acc, item) => acc + Number(item.quantity || 0), 0);
  const totalSold = sales.reduce((acc, item) => acc + Number(item.quantity || 0), 0);

  return `
    <section class="split-grid">
      <article class="card section-card">
        <h2>Registrar entrada do fornecedor</h2>
        ...
        <div class="form-actions">
          <button id="btn-save-purchase" class="btn btn-primary">Salvar entrada</button>
        </div>
      </article>

      <article class="card section-card">
        <h2>${editingStockItem ? 'Editar estoque do produto' : 'Resumo do estoque'}</h2>
        ${editingStockItem ? `
          <div class="field-group">
            <label for="stock-adjust-product">Produto</label>
            <input id="stock-adjust-product" type="text" value="${escapeHtml(editingStockItem.productName || '')}" readonly />
          </div>
          <div class="field-grid two-columns">
            <div class="field-group">
              <label for="stock-adjust-current">Estoque atual</label>
              <input id="stock-adjust-current" type="number" value="${Number(editingStockItem.availableQuantity || 0)}" readonly />
            </div>
            <div class="field-group">
              <label for="stock-adjust-new">Novo estoque</label>
              <input id="stock-adjust-new" type="number" min="0" value="${Number(editingStockItem.availableQuantity || 0)}" />
            </div>
          </div>
          <div class="field-group">
            <label for="stock-adjust-notes">Observações</label>
            <textarea id="stock-adjust-notes" rows="3" placeholder="Ex.: conferência manual, ajuste de contagem, perda ou sobra"></textarea>
          </div>
          <p class="small-text">Esse ajuste altera diretamente o saldo atual do produto selecionado.</p>
          <div class="form-actions">
            <button id="btn-save-stock-adjustment" class="btn btn-primary">Salvar ajuste</button>
            <button class="btn btn-ghost" data-action="cancel-edit-stock">Cancelar</button>
          </div>
        ` : `
          <div class="kpi-panel">
            <div class="kpi-line"><span>Estoque atual</span><strong>${totalAvailable} un.</strong></div>
            <div class="kpi-line"><span>Entradas registradas</span><strong>${totalPurchased} un.</strong></div>
            <div class="kpi-line"><span>Saídas por venda</span><strong>${totalSold} un.</strong></div>
            <div class="kpi-line"><span>Regra</span><strong>Venda baixa o estoque automaticamente</strong></div>
          </div>
          <p class="small-text" style="margin-top: 12px;">Use o botão de editar na tabela para corrigir o saldo de um produto quando precisar.</p>
        `}
      </article>
    </section>
    ...
    <section class="card section-card">
      ...
      <p class="small-text">Aqui mostramos apenas o saldo atual. Não há histórico nessa tabela.</p>
            ...
            <tbody>
              ${stockSummary.map((item) => `
                <tr>
                  ...
                  <td>${Number(item.purchasedQuantity || 0)}</td>
                  <td>${Number(item.soldQuantity || 0)}</td>
                  <td><strong>${Number(item.availableQuantity || 0)}</strong></td>
                  <td>${Number(item.availableQuantity || 0) > 0 ? '<span class="pill success">Disponível</span>' : '<span class="pill warning">Sem saldo</span>'}</td>
                  <td>
                    <div class="table-actions">
                      <button class="btn-inline" data-action="edit-stock" data-product="${escapeHtml(item.productName || '')}">Editar estoque</button>
                    </div>
                  </td>
                </tr>
              `).join('')}
      ...
      ` : renderEmptyState('Nenhum produto encontrado para controle de estoque.')}
    </section>
  `;
}
