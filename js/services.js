...
export async function removeVendor(uid) {
  ...
  const refsToClean = [
    ...
    query(collection(db, collections.stockAdjustments), where('ownerId', '==', uid))
  ];
...
export async function fetchPurchases(ownerId = null) {
  ...
  return snap.docs
    .map(normalizeDoc)
    .filter((item) => item.entryType !== 'stock_adjustment')
    .sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0));
}

function isPermissionError(error) {
  const code = error?.code || '';
  return code.includes('permission-denied') || /permission/i.test(error?.message || '');
}

function normalizePurchaseAdjustment(snapshot) {
  ...
  return {
    ...
    deltaQuantity: Number(data.deltaQuantity || 0),
    notes: data.notes || '',
...
export async function createStockAdjustment(ownerId, payload) {
  const currentQuantity = Number(payload.currentQuantity || 0);
  const newQuantity = Number(payload.newQuantity || 0);
  ...
  try {
    return await addDoc(collection(db, collections.stockAdjustments), adjustmentPayload);
  } catch (error) {
...
export async function fetchStockAdjustments(ownerId = null) {
  const adjustments = [];

  try {
    const base = collection(db, collections.stockAdjustments);
    const q = ownerId ? query(base, where('ownerId', '==', ownerId)) : query(base);
    const snap = await getDocs(q);
    adjustments.push(...snap.docs.map(normalizeDoc));
  } catch (error) {
    if (!isPermissionError(error)) throw error;
  }

  const purchasesBase = collection(db, collections.purchases);
  const purchasesQuery = ownerId ? query(purchasesBase, where('ownerId', '==', ownerId)) : query(purchasesBase);
  const purchasesSnap = await getDocs(purchasesQuery);
  adjustments.push(
    ...purchasesSnap.docs
      .filter((docSnap) => (docSnap.data() || {}).entryType === 'stock_adjustment')
      .map(normalizePurchaseAdjustment)
  );
...
export async function deleteSale(ownerId, saleId) {
  ...
  if (data.ownerId !== ownerId) {
    throw new Error('Permissão insuficiente para excluir esta venda.');
  }
...
export function computeVendorMetrics({ sales, purchases, expenses, stockAdjustments = [] }) {
  ...
  const stockUnits = purchases.reduce((acc, item) => acc + Number(item.quantity || 0), 0)
    ...
    + stockAdjustments.reduce((acc, item) => acc + Number(item.deltaQuantity || 0), 0);
...
export function buildStockSummary(purchases = [], sales = [], stockAdjustments = []) {
  const productNames = new Set([
    ...PLATFORM_CONFIG.defaultCatalog,
    ...purchases.map((item) => item.productName).filter(Boolean),
    ...sales.map((item) => item.productName).filter(Boolean),
    ...stockAdjustments.map((item) => item.productName).filter(Boolean)
  ]);
  ...
  return Array.from(productNames).map((productName) => {
    ...
    const adjustmentQuantity = stockAdjustments
      .filter((item) => item.productName === productName)
      .reduce((acc, item) => acc + Number(item.deltaQuantity || 0), 0);
    ...
    return {
      ...
      soldQuantity,
      adjustmentQuantity,
      availableQuantity: purchasedQuantity - soldQuantity + adjustmentQuantity
    };
