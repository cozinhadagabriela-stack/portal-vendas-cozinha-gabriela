...
async function handleSaveStockAdjustment() {
  ...
  const stockInput = $('#stock-adjust-current');
  const currentQuantity = Number(stockInput?.dataset.originalValue || 0);
  const newQuantity = Number(stockInput?.value || 0);
  ...
  if (!Number.isFinite(newQuantity) || newQuantity < 0) {
    alert('Informe um estoque atual válido.');
    return;
  }
