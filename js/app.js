...
async function refreshPlatformBadge() {
  ...
  if (state.profile?.role === 'admin') {
    ...
    platformBadge.textContent = `Limite: ${settings.maxActiveVendors} vendedores`;
  }
}
...
function renderAdminView() {
  ...
  const map = {
    ...
    'admin-vendors': {
      title: 'Vendedores',
      html: renderAdminVendorsView(state.cache.vendors, vendorSummary, state.platformSettings)
    },
    'admin-notices': {
      title: 'Informativos',
...
async function bootstrapSession(user) {
  ...
  try {
    ...
    setMessage(error.message || 'Não foi possível carregar a sessão.', 'error');
  }
}
